...
import {compatJamoToStandardJamo, standardJamoToCompatJamo, compatJamoToStandardJamoInSyllable} from '../HangulJamo'
...
export default class Hangul2Keyboard extends SimpleKeyboard {
    ...
    convertFrom(input: string): number[] {
        return super.convertFrom(standardJamoToCompatJamo(this.decombinate(compatJamoToStandardJamo(input.normalize('NFD')))))
    }
    convertTo(input: number[]): string {
        return standardJamoToCompatJamo(this.combinate(compatJamoToStandardJamoInSyllable(super.convertTo(input))).normalize('NFC'))
    }
